...
#     gen_step = 0
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        if args.loss == 'hinge':
            ...
            #soft label
            real_label = torch.rand((imgs.shape[0],), dtype=torch.float, device=real_imgs.get_device()) * 0.2 + 0.8  # مقدار تصادفی بین 0.8 تا 1.0
            fake_label = torch.rand((imgs.shape[0],), dtype=torch.float, device=real_imgs.get_device()) * 0.2       # مقدار تصادفی بین 0.0 تا 0.2
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        if global_steps % (args.n_critic * args.accumulated_times) == 0:
            ...
            for accumulated_idx in range(args.g_accumulated_times):
                ...
                freq_loss = gen_net.module.natural_frequency_loss(real_imgs, gen_imgs)
                ...
                g_loss += args.lambda_freq * freq_loss
