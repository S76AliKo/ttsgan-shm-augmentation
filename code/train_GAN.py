...
#     test_loader = data.DataLoader(test_set, batch_size=args.dis_batch_size, num_workers=args.num_workers, shuffle=True)
    ...
    test_loader = data.DataLoader(test_set, batch_size=args.batch_size, num_workers=args.num_workers, shuffle = True, drop_last=True)
    ...
    # initial
    fixed_z = torch.cuda.FloatTensor(np.random.normal(0, 1, (256, args.latent_dim)))
...
def gen_plot(gen_net, epoch, class_name):
    ...
    for i in range(10):
        fake_noise = torch.FloatTensor(np.random.normal(0, 1, (1, 256)))
